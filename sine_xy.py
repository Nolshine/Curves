...
phase1 = 1 # phase is a value that is going to range from 1 to 360, and it's going to control the point's height.
...
while (not stop):
    ...
    if phase2 > 360:
        phase2 = phase2-360
    x = px+math.sin(math.radians(phase2))*SCREEN_WIDTH
    if x > SCREEN_WIDTH:
        x = x-SCREEN_WIDTH
    if x < 0:
        x = x+SCREEN_WIDTH
    y = py+math.sin(math.radians(phase1))*AMPLITUDE
